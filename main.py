...
from extractor import get_pdf_files, get_pdf_name, get_page_count, extract_page
from ai_client import analyze_page
from notes_writer import append_page_notes, get_notes_path
from prompts import SYSTEM_PROMPT, PAGE_PROMPT
from stats import get_pdf_text_tokens, get_notes_tokens, get_notes_page_count
...
def display_pdf_list(pdfs: list[Path], token_cache: dict[str, int]) -> None:
    ...
    with Live(_build_pdf_table(pdfs, token_cache), console=console, refresh_per_second=4) as live:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {executor.submit(_count_tokens, pdf): pdf for _, pdf in to_compute}
...
def process_pages(
        ...
        for page_num in pages:
            ...
            try:
                ...
                user_prompt = PAGE_PROMPT.format(
                    ...
                    page_text=text if text.strip() else "(No extractable text — rely on the image.)",
                )
                ...
                if was_skipped:
                    skipped += 1
                    progress.update(task, status=f"Page {page_num} (skipped — already exists)")
                else:
                    processed += 1
    ...
    console.print(Panel(summary, title="[title]Extraction Complete[/title]", box=box.ROUNDED, border_style="green"))
...
def display_token_stats(pdfs: list[Path]) -> None:
    ...
    table.add_row(
        ...
        "",
        f"[bold]{total_notes_tokens:,}[/bold]" if total_notes_tokens else "[dim]—[/dim]",
        f"{total_notes_tokens / total_pdf_tokens:.1%}" if total_pdf_tokens > 0 and total_notes_tokens > 0 else "—",
    )

    console.print()
    console.print(table)
    console.print("[info]Tokens counted using cl100k_base encoding (GPT-4 / general approximation)[/info]")
    console.print()
...
def main():
    ...
    while True:
        ...
        # Show current mode
        mode_label = "[success]ON[/success] (text + image)" if send_image else "[warning]OFF[/warning] (text only)"
        ...
        if choice.lower() == "i":
            ...
            state = "[success]ON[/success] — sending text + image" if send_image else "[warning]OFF[/warning] — sending text only"
        ...
        if pages is None:
            console.print(f"[error]Invalid range. Must be between 1 and {total_pages}.[/error]\n")
            continue

        console.print(f"[info]Will process {len(pages)} page(s): {pages[0]}–{pages[-1]}[/info]")

        # Confirm
        confirm = Prompt.ask("[warning]Start extraction?[/warning]", choices=["y", "n"], default="y")
        if confirm.lower() != "y":
